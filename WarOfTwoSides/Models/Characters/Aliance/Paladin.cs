...
{
	public class Paladin : ChurchWarriors
	{
		...
		(
			name,
			Default_Value.ALIANCE_DEFAULT_HEALTH + Default_Value.PALADIN_HEALTH,
			Default_Value.ALIANCE_DEFAULT_STRENGHT + Default_Value.PALADIN_STRENGHT,
			Default_Value.ALIANCE_DEFAULT_ATTACK_DEMAGE + Default_Value.PALADIN_DEFAULT_ATTACK_DEMAGE,
			Default_Value.ALIANCE_DEFAULT_MAGIC_DEMAGE + Default_Value.PALADIN_DEFAULT_MAGIC_DEMAGE,
			Default_Value.ALIANCE_DEFAULT_FACTION,
			Stances.attack
		)
		{

			base.Holy = Default_Value.PALADIN_DEFAULT_HOLY;
		...
		public override void Attack(IWarrior warrior)
		{
			...
			else
			{
				...
				if (!(Weapon.Durability < 1))
				{
					...
					Console.WriteLine("Total magic demage fuzed with holy: " + totalMagicDemage);
