...
{
	public class DemonHunter : Warrior
	{
		private int _demonKilled;

		public int DemonKilled 
		{
			get => this._demonKilled;
			set => this._demonKilled = value; 
		}


		public DemonHunter(string name = "Demon Hunter")
		: base
		(
			name,
		...
		{
			this.DemonKilled = 0;
			this.Weapon = new Warglaives();
			this.Arrmor = new LigtArmor();
		}
		...
		public override void Attack(IWarrior warrior)
		{
			...
			else
			{
				...
				if (!(Weapon.Durability < 1))
				{
					...
					totalDemage = totalAttackDemage + totalMagicDemage + DemonKilled * DemonKilled;
					Console.WriteLine("Total demon killed for bonus demage: " + DemonKilled);
