...
namespace WarOfTwoSides.Models.Characters.Demons
{
    public class Demon : Warrior
    {
		...
		public Demon(string name = "Demon")
		: base
		(
			name,
		...
		{
			this.Killed = 0;
			base.Arrmor = new DemonSkin();
			base.Weapon = new DemonClaw();
		}
		...
		public override void Attack(IWarrior warrior)
		{
			...
			else
			{
				...
				if (!(Weapon.Durability < 1))
				{
					...
					totalDemage += totalAttackDemage + totalMagicDemage + Killed * Killed;

					Console.WriteLine("Total warriors killed for bonus demage: " + this.Killed);
